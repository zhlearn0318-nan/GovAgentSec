...
test("security center provides overview and all five feature tabs", () => {
  ...
  assert.match(html, /Aegis 供应链安全中心/);
  assert.doesNotMatch(html, /Aegis 安全中心/);
...
test("plugin stays embedded without a standalone OpenClaw sidebar tab", () => {
  const source = readFileSync(new URL("./index.js", import.meta.url), "utf8");
  assert.equal((source.match(/registerControlUiDescriptor\s*\(\s*\{/g) || []).length, 0);
  assert.doesNotMatch(source, /label:\s*"Aegis 安全中心"/);
});
