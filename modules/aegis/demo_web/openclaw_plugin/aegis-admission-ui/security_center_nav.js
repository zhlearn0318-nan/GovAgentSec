...
export function renderSecurityCenterNav(activeTab) {
  return `<nav class="aegis-center-nav" aria-label="Aegis 供应链安全中心功能导航">${ITEMS.map(([id, label, href]) => `<a class="nav-button${id === activeTab ? " active" : ""}" data-tab="${id}" href="${href}"${id === activeTab ? ' aria-current="page"' : ""}><span class="nav-icon" aria-hidden="true">${ICONS[id]}</span><span>${label}</span></a>`).join("")}</nav>`;
}
